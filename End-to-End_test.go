...
func (suite *E2ETestSuite) TestCompleteApplicationWorkflow() {
	suite.Run("Complete workflow from registration to task management", func() {
		...
		// Step 8: Newly promoted admin creates a task
		...
		w = suite.makeRequest("POST", "/tasks", newAdminTask, userToken)
		suite.Equal(http.StatusCreated, w.Code)

		var newTask domain.Task
		suite.parseResponse(w, &newTask)
		suite.Equal("Task by Promoted Admin", newTask.Title)
